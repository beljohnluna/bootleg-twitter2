...
function Post({id, post, postPage}) {
  ...
  //built-in router nextjs very useful indeed
  const router = useRouter();
  
  //basically displays the number of likes in a post
    ...
    <div className="p-3 flex cursor-pointer border-b border-gray-200" onClick={() => router.push(`/${id}`)}>
        ...
        <div className="flex flex-col w-full space-y-2">
            <div className={`flex ${!postPage && "justify-between"}`}>
              ...
              <div className="text-[#7e757d]">
                <div className="inline-block group">
                  <h4 className={`font-bold text-[15px] sm:text-base text-black group-hover:underline ${!postPage && "inline-block"}`}
                  >{post?.username}</h4>
                ...
                {!postPage && (
                <p className="text-[black] text-[15px] mt-0.5 sm:text-base">
                  {post?.text}
                </p>
                )}
              </div>
        ...
        >
          <div
            ...
            onClick={(e) => {
              e.stopPropagation(); setPostId(id); setIsOpen(true);
            }}
          >
